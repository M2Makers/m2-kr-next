...
2007년 역사적인 프리젠테이션 이후 엔터프라이즈 환경에서도 혁신적인 변화가 이루어졌다.
...
하지만 안타깝게도 이런 혁신적인 변화와는 다르게 **Workload** 를 처리하는 엔터프라이즈의 보수적인 인식은 크게 변화지 않았다.
그 중심에는 **Batch Processing (이하 Batch)** 이 다양한 형태로 자리잡고 있었다. 

우리는 **M2** 를 통해 더 나은 **Workload Processing** 방향을 제시하고자 한다.
...
.. note::
   ...
   컨텐츠는 Batch가 아닌, 요청 즉시 실시간으로 처리되어야 합니다. 
   우리는 이런 방식을 On-the-fly라고 합니다.
...
우리가 소비하고 있는 대부분 온라인 컨텐츠는 휘발성이다. 

이커머스(E-Commerce)를 예로 들어 보자.
이커머스에서는 상품판매를 위해 상품사진과 설명을 사용자에게 노출하는 방식을 사용한다.
변화에 빠르게 대응할 수 있어야 판매량을 지속시킬 수 있다.
...
많은 연구 자료가 급격하게 떨어지는 소비율 패턴을 증명한다.
...
하지만, 상품 이미지를 다루는 방식은 마치 그 자체가 상품인 것처럼 소중히 다루는 경향이 있다.
여기에 괴리가 있다.

이커머스에서 이미지는 소비되는 특성에 맞게 다뤄져야 한다. 
그러나 여전히 많은 이커머스에서 이미지는 여전히 고전적인 Batch에 의존하고 있다. 
심지어 클라우드 서비스들, 시장의 다양한 이미지 솔루션/서비스들이 Batch 방식을 더 고집하게 만들어 효율성과 기민함을 저하시키고 있다.

이제는 더 이상 이커머스에서 Batch 방식은 통하지 않는다. 
사전에 기획된 상품을 사용자에게 노출하는 것이 아닌, 다수의 셀러나 제휴사업자들이 상품을 등록하고 판매하는 오픈마켓의 성격으로 확장되고 있기 때문입니다.
...
서비스가 커질수록 변동성/불확실성은 커지게 되고 임계점을 넘어가는 순간 지금까지 유효했던 해법들은 한계를 드러낸다. 
이 부채를 갚지 못한다면 영원한 이자를 내야 한다.
...
Batch의 문제와 닮아있는 컨테이너 운송, 모든 운송에 컨테이너를 쓰지 않는다. 
컨테이너에 적합한 운송 모델이 있는 것일 뿐이다.
...
컨테이너가 단 한 개라면 문제가 단순하지만, 두 개의 컨테이너가 상단으로 적재되더라도 해법이 만만치 않다.  
...
파이프라인 수송(Oil Pipeline Transport)의 접근 방식은 ``M2`` 에서 추구하는 On-the-fly 컨셉과 매우 닮아 있다. 
...
연료를 운반하는 방식에서 유조선은 Batch, 오일 파이프라인을 On-the-fly로 대조해 볼 수 있다. 
...
자연히 관리 비용이 급격하게 높아지게 될 뿐만 아니라, 사고에 대한 리스크도 커지게 됩니다.
...
``M2`` 는 초기 제품 컨셉에서 부터 Batch 방식의 문제점을 일찍부터 깨닫고 모든 기능들이 On-the-fly 아키텍쳐로 동작한다.
...
.. note::
   ...
   연료가 타고 사라지는 것처럼 이커머스 이미지도 목적을 다하면 타고 사라져야 한다.. 
   강력한 연소(=on the fly 프로세싱)과 사라짐(Stateless) 이 것이 ``M2`` 의 본질이다.
   

단어에 주의하라. 
이 시장엔 끊임없이 단어 만들기를 좋아하는 사람들이 ``Batch Pipeline`` 이란 용어를 만들었다. 
그런데, 이 데이터 분석에서 활용하는 ``Batch Pipeline`` 은 실시간 콘텐츠 처리의 On-the-fly 와는 다르다.


결론. On-the-fly 관점으로의 전환
===================

비즈니스 문제를 On-the-fly의 관점에서 바라보면 정말 경제적인 해답을 찾을 수 있다. 
더 놀라운 점은 기존의 인프라 환경을 변화시키지 않고도 실천할 수 있다는 점이다.
