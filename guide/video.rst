...
비디오 엔진은 비디오/오디오를 ``on the fly`` 로 가공하여 다양한 형태로 변환시킨다.
...
화질 상승은 더 큰 동영상을 생성하며 더 많은 대역폭과 저장공간을 필요로 한다.
``M2`` 는 다양한 On-the-fly 기법을 이용해 기존 Back-End의 수정없이 유연한 전송기능을 제공한다. 
...
이런 경우 M2 역시 원본 파일을 그대로 복제하여 고객에게 서비스한다.
...
다음 클라이언트 요청에 대해 M2가 어떻게 동작하는지 이해해보자. ::

   GET /video.mp4/mp4hls/99.ts HTTP/1.1
   Range: bytes=0-512000
   Host: www.winesoft.co.kr

1.	``M2`` 최초 로딩 (아무 것도 캐싱되어 있지 않음.)
#.	``Client`` HTTP Range 요청 (100번째 파일의 최초 500KB 요청)
#.	``M2`` /video.mp4 파일 캐싱객체 생성
#.	``M2`` /video.mp4 파일 분석을 위해 필요한 부분만을 원본서버에서 다운로드
#.	``M2`` 100번째(99.ts)파일 서비스를 위해 필요한 부분만을 원본서버에서 다운로드
#.	``M2`` 100번째(99.ts)파일 생성 후 Range 서비스
#.	``M2`` 서비스가 완료되면 99.ts파일 파괴
